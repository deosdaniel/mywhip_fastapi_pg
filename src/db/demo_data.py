...
def generate_expenses(count: int):
    ...
    expense_names = [
        "Покраска", "Замена масла", "Ремонт подвески",
        "Шиномонтаж", "Химчистка", "Полировка",
        "Замена стекла", "Ремонт АКПП", "Тонировка"
    ]
    return [
        Expenses(
            name=random.choice(expense_names),
            exp_summ=random.randint(5000, 50000)
...
def generate_car(status: str):
    ...
    car = Cars(
        uid=uuid4(),
        make=fake.random_element(["Toyota", "BMW", "Mercedes", "Audi", "Ford", "Kia", "Hyundai"]),
        model=fake.random_element(["Camry", "X5", "C-Class", "A6", "Focus", "Rio", "Solaris"]),
        year=str(fake.date_between(start_date='-13y', end_date='today').year),  # Исправлено здесь
        vin=fake.vin(),
    ...
    if status in [CarStatusChoices.SOLD, CarStatusChoices.LISTED]:
        ...
        if status == CarStatusChoices.SOLD:
            ...
            car.price_sold = random.randint(
                int(listed_price * 0.9),
                listed_price + random.randint(0, 100000)
            )
...
def generate_demo_cars():
    ...
    for _ in range(17):  # Без расходов
        status = random.choice([CarStatusChoices.FRESH, CarStatusChoices.REPAIRING, CarStatusChoices.DETAILING])
